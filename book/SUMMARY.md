# Summary

* [引言](README.md)

* [第一章 后端技术导言](chapter1-servertech/README.md)
    - [1.1 后端基础设施](chapter1-servertech/server-basic.md)
    - [1.2 Java后端技术概览](chapter1-servertech/server-tech-tree.md)
    - [1.3 如何学习后端技术](chapter1-servertech/how-to-study.md)

* [第二章 Java项目工程化](chapter2-project/README.md)
    - [2.1 项目构建](chapter2-project/build.md)
    - [2.2 代码版本控制](chapter2-project/vcs.md)
    - [2.3 代码质量保证](chapter2-project/quality.md)
      
* [第三章 开发框架](chapter3-framework/README.md)
    - [3.1 依赖注入](chapter3-framework/ioc.md)
    - [3.2 对象关系映射](chapter3-framework/orm.md)
    - [3.3 日志](chapter3-framework/log.md)
    - [3.4 Web MVC](chapter3-framework/mvc.md)
    - [3.5 响应式Web框架](chapter3-framework/reactive-web.md)

* [第四章 Spring](chapter4-spring/README.md)
    - [4.1 Spring核心组件](chapter4-spring/spring.md)
    - [4.2 Spring数据操作框架](chapter4-spring/spring-data.md)
    - [4.3 使用Spring Boot快速开发](chapter4-spring/spring-boot.md)
    - [4.4 Spring常用组件](chapter4-spring/spring-common.md)
    - [4.5 总结](chapter4-spring/end.md)
   
* [第五章 数据存储](chapter5-datastore/README.md)
    - [5.1 关系型数据库-MySQL](chapter5-datastore/rds.md)
    - [5.2 非关系型数据库](chapter5-datastore/nosql.md)
    - [5.3 缓存](chapter5-datastore/cache.md)
    - [5.4 搜索引擎-Elasticsearch](chapter5-datastore/search.md)
   
* [第六章 数据通信](chapter6-datatrans/README.md)
    - [6.1 RESTful架构风格](chapter6-datatrans/rest.md)
    - [6.2 远程过程调用-RPC](chapter6-datatrans/rpc.md)
    - [6.3 消息中间件](chapter6-datatrans/message.md)
   
* [第七章 Java编程进阶](chapter7-java/README.md)
    - [7.1 Java内存管理](chapter7-java/java-mm.md)
    - [7.2 Java网络编程](chapter7-java/network.md)
    - [7.3 Java并发编程](chapter7-java/concurrent.md)
    - [7.4 Java开发利器](chapter7-java/weapons.md)
    - [7.5 Java新版本特性](chapter7-java/newjava.md)
    - [7.6 总结](chapter7-java/end.md)

* [第八章 性能调优](chapter8-profile/README.md)
    - [8.1 调优准备](chapter8-profile/ready.md)
    - [8.2 性能分析](chapter8-profile/analysis.md)
    - [8.3 性能调优](chapter8-profile/chapter8-profile.md)

* [第九章 安全技术](chapter9-security/README.md)
    - [9.1 Java加密](chapter9-security/java-encrypt.md)
    - [9.2 安全HTTP - HTTPS](chapter9-security/https.md)
    - [9.3 Web安全](chapter9-security/web-security.md)

* [附录A: 代码构建常用命令](appendix/build-cmd.md)

* [附录B: Git常用命令](appendix/git-usage.md)

* [附录C: MySQL常用命令](appendix/mysql-usage.md)

* [附录D: MongoDB常用命令](appendix/mongo-usage.md)

* [附录E: Java调优常用命令](appendix/java-profile.md)

* [附录F: 架构简明指南](appendix/arch-usage.md)