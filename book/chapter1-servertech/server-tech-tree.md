...
基于1.1节介绍的后端基础设施，可以从中引出其需要的技能列表。进一步细化可以得到更为具体的技能树，也是Java后端工程师技能的一个总体概览图：
...
- 虚拟化：虚拟化技术是前几年经常用来做私有云的一种技术。即将自己的物理主机通过虚拟化技术分裂为多个虚拟主机，能够隔离资源。其中，VPS（虚拟专用服务器）的代表技术包括：微软的Virtual Server、VMware的ESX Server、SWsoft的Virtuozzo。此外，OpenStack提供的构建私有IIAS的功能、Cloud Foundry提供的构建私有平台运行环境以及Docker、Kubernetes带来的容器服务都是虚拟化技术的一种。
...
一个Web应用从0开始一般会经历单体应用、垂直应用到分布式服务架构的演化（具体的会随着业务的不同有所不同）。如下图所示：
...
![](media/three-server-tech.png)

- 业务研发：主要负责的是业务逻辑的开发，通常关注的就是开发效率、接口性能、研发流程等。面对的挑战主要是高并发、大流量等挑战。
- 大数据：随着业务量的上升，数据的不断增加，需要依赖大数据技术来提高数据处理效率，从而进一步促进业务的增长。包括数据仓库构建、实时数据分析方面的研发。面对的挑战主要是数据采集、数据清洗、海量数据处理、数据仓库高效存取等。
- 数据挖掘：挖掘数据金矿，利用数据服务于业务。虽然现在很多说法都是和大数据一起讲的，其实数据挖掘和大数据并没有必然联系，只不过海量数据能够给予数据挖掘更加丰富的训练数据，从而使得模型更准确。面对的挑战主要是各种数据挖掘算法、模型的构建、使用和调优。

对于大的团队来说，还会有专门的团队负责基础框架、中间件、基础设施（运维、云计算、测试）的研发。
